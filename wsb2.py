...
render the page, intercept that response, decode it, and extract:
  ...
  - daily_comment_metrics              list[float]  (hourly comment counts x 24)
  - hourly_comment_metrics             list[float]

Usage:
    from wsb2 import fetch_wsbapp_post, get_latest_wsbapp_data
...
def _capture_devvit_proto(post_url: str) -> bytes:
    ...
    with sync_playwright() as p:
        ...
        # then navigate (domcontentloaded only — Reddit never reaches networkidle).
        with page.expect_response(
            lambda r: "RenderPostContent" in r.url,
            timeout=45_000,
        ) as response_info:
    ...
    return _strip_grpc_frame(body)


# ---------------------------------------------------------------------------
# Minimal protobuf wire-format decoder (no schema needed)
# ---------------------------------------------------------------------------
...
def _decode_proto(buf: bytes) -> dict:
    """Decode a protobuf binary blob without a schema.

    Length-delimited fields are returned as raw bytes unless they parse as a
    complete, valid embedded message (all bytes consumed, no unknown wire types).
    This correctly distinguishes string/bytes fields from nested messages.
    """
    msg: dict = {}
    pos, n = 0, len(buf)
    while pos < n:
        ...
        if wire == 0:
            ...
            val = int.from_bytes(buf[pos:pos+8], 'little'); pos += 8
        elif wire == 2:
            length, pos = _read_varint(buf, pos)
            raw = buf[pos:pos+length]; pos += length
            val = _try_as_message(raw)
        elif wire == 5:
            val = int.from_bytes(buf[pos:pos+4], 'little'); pos += 4
        else:
            break   # unknown wire type → stop (caller will get partial parse)
        key = str(field)
        if key in msg:
...
def _try_as_message(buf: bytes):
    """Return a decoded dict if buf is a valid complete protobuf message, else raw bytes."""
    if not buf:
        return buf
    pos, n = 0, len(buf)
    out: dict = {}
    try:
        while pos < n:
            ...
            if wire == 0:
                ...
                if pos + 8 > n: return buf
                val = int.from_bytes(buf[pos:pos+8], 'little'); pos += 8
            elif wire == 2:
                length, pos = _read_varint(buf, pos)
                if pos + length > n: return buf
                raw = buf[pos:pos+length]; pos += length
                val = _try_as_message(raw)
            elif wire == 5:
                if pos + 4 > n: return buf
                val = int.from_bytes(buf[pos:pos+4], 'little'); pos += 4
            else:
                return buf  # unknown wire type → not a valid message
            key = str(field)
            if key in out:
                ...
                out[key] = val
        return out if pos == n else buf   # only accept if all bytes consumed
    except Exception:
        return buf
...
def save_wsb_data(data: dict) -> None:
    ...
    date = post_date.date() if isinstance(post_date, datetime) else pd.Timestamp(post_date).date()
